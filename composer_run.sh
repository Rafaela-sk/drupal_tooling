...
echo "./vendor/drush/drush/drush webform-libraries-download"
echo ""
echo "### BACKUP ###"
...
echo "./vendor/drush/drush/drush archive:dump --db --destination=~/${dirname}_drush_sql_`date +'%Y%m%d'`.tar.gz"
echo "cp sites/default/settings.php ~/${dirname}_drush_`date +'%Y%m%d'`.settings.php"
echo "tar -czf ~/${dirname}_`date +'%Y%m%d'`.tar.gz `pwd`"
echo ""
echo "### RESTORE ###"
